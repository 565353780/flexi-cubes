...
class FlexiCubes:
    ...
    def __call__(self, voxelgrid_vertices, scalar_field, cube_idx, resolution, qef_reg_scale=1e-3,
        ...
        vd, L_dev, vd_gamma, vd_idx_map, vd_features = self._compute_vd(
            ...
            case_ids, beta, alpha, gamma_f,
            idx_map, grad_func, qef_reg_scale, voxelgrid_features
        )
    ...
    def _triangulate(self, scalar_field, surf_edges, vd, vd_gamma, edge_counts, idx_map, vd_idx_map, surf_edges_mask, training, grad_func, vd_features):
        ...
        if not training:
            ...
            # print("vd_quad: ", vd_quad.shape, quad_vd_idx.shape )
            # print("vd_02: ", vd_02.shape, vd_13.shape )
            if vd_features is not None:
                ...
                # print("features_quad: ", features_quad.shape , quad_vd_idx.shape ) 
                # features_02 = (features_quad[:, 0] + features_quad[:, 2]) / 2
                # features_13 = (features_quad[:, 1] + features_quad[:, 3]) / 2 

                features_02 = (torch.index_select(input=features_quad, index=torch.tensor(0, device=self.device), dim=1) +
                     torch.index_select(input=features_quad, index=torch.tensor(2, device=self.device), dim=1)) / 2
                features_13 = (torch.index_select(input=features_quad, index=torch.tensor(1, device=self.device), dim=1) +
                     torch.index_select(input=features_quad, index=torch.tensor(3, device=self.device), dim=1)) / 2
            
                features_center = ((features_02 * gamma_02.unsqueeze(-1) + features_13 * gamma_13.unsqueeze(-1)) /
                                    weight_sum.unsqueeze(-1)).squeeze(1) 
                
                # print(vd_features.shape, features_center.shape )
